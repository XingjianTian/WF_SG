...
func init() {
	...
	if SessManager != nil {
		fmt.Println("SessManager Created")
	}
}
...
获取分页html
*/
func GetPageHtml3(totalPages, currentPage, total int, path string) string {
	list := ""
	...
	//indication:="<li class='text'> All " + strconv.Itoa(total) + " records / " + strconv.Itoa(totalPages)+" pages </li> "

	html := "<div class='card-footer clearfix'><em>All " + strconv.Itoa(total) + " records / " + strconv.Itoa(totalPages) + " pages</em>" +
...
func GetPageHtml(totalPages, currentPage, total int, path string) string {
	...
	for i := 1; i <= totalPages; i++ {
		...
		list += "<li class='page-item " + active + "'><a class='page-link' href='" + path + "?page=" + strconv.Itoa(i) + "'>" + strconv.Itoa(i) + "</a></li> "
	}
	...
	if currentPage == 1 {
		...
		previous = "<li class='page-item active'><a class='page-link' href='" + path + "?page=" + strconv.Itoa(currentPage-1) + "'>Previous</a></li> "
	}
	...
	if currentPage >= totalPages {
		...
		next = "<li class='page-item active'><a class='page-link' href='" + path + "?page=" + strconv.Itoa(currentPage+1) + "'>Next</a></li> "
	}
...
func DefaultErrorShow(msg string, Ctx iris.Context) {
	...
	Ctx.ViewData("Title", "Message Error")
